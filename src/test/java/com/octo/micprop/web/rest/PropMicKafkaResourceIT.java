package com.octo.micprop.web.rest;
...
import com.octo.micprop.config.KafkaProperties;
...
class PropMicKafkaResourceIT {
    ...
    void setup() {
        ...
        PropMicKafkaResource kafkaResource = new PropMicKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/prop-mic-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/prop-mic-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
